...
| 4. Validation | Evidence supporting the results and their limits | Comparisons, robustness checks, and report traceability meet stated criteria |
...
Validation may add targeted checks: amend the spec with their purpose and expected outputs before running them. If this changes scientific scope, reopen Exploration; if it requires new implementation, reopen Distillation. Never advance or reopen silently.
...
| Step 9 — final verification | Distillation establishes engineering evidence; Validation reuses valid evidence and adds scientific checks. |
